...
import com.example.dt.testapp3.VisitNetworks.GodLiuCensorTapeDownloader;
import com.example.dt.testapp3.VisitNetworks.VisitHTTPUploader;
import com.example.dt.testapp3.VisitNetworks.VisitJSONOperate;
import com.example.dt.testapp3.VisitNetworks.VisitSyncNewApproval;
...
//import static com.example.dt.testapp3.Graphics.GodLiuStructure.*;
...
//        };
    ...
    public void setIntoContentFlag() {
        currentFg = 1;
    }
    ...
    public void setAddAndSaveVisible(boolean kms){
        if (addItem != null)
            addItem.setVisible(kms);
    }

    public void setAddAndSaveText(String str){
        if (addItem != null)
            addItem.setTitle(str);
    }

    public void setSubmitVisible(boolean kms){
        submitItem.setVisible(kms);
    }

    public void setFinishVisible(boolean kms){
        finishItem.setVisible(kms);
    }

    public void setTapeVisible(boolean kms) {
        tapeItem.setVisible(kms);
    }

    public void setPlayVisible(boolean kms) {
        playItem.setVisible(kms);
    }

    public void setAddFlag(boolean kms) {
        isAdding = kms;
    }
...
//        submitItem.setVisible(false);
        switch (currentFg) {
            ...
            case 1:
                ...
                if (checkTaping())
                    return;
                if (visitContentFragment.getCondition()>=4){
                    backFromContent();
                    return;
                }
...
//                Log.e("/sdcard/", Environment.getExternalStorageDirectory().getPath());
            ++MaxTapeId;
            name = String.format("tape_%d_%d_%d.amr", Integer.parseInt(UID), (isAdding ? MaxId + 1 : visitContentFragment.getVisitId()), MaxTapeId);
...
//                format : tape+uid(%4d)+id(%8d)+number(%4d);
            Log.e("directory : ", dir);
            recorder.setOutputFile(dir + File.separator + name);
            // 设置录制好的音频文件保存路径
    ...
    private void clickPlay() {
        if (playItem.getTitle().equals("播放")) {
            ...
            boolean b = tapeSpinner.isSelected();
            Object obj = tapeSpinner.getSelectedItem();
            if (obj == null)
                return;
            String str = obj.toString();
            player = new MediaPlayer();
            try {
                String str2 = String.format("%s/SaleCircle", Environment.getExternalStorageDirectory().getPath());
                File f = new File(str2);
                if (!f.exists())
                    f.mkdirs();
                Log.e("player data source", str2 + File.separator + str);
                player.setDataSource(str2 + File.separator + str);
                player.prepare();
                player.start();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            ...
            case 2:
                ...
                clickAddAndSave(true, true, user.id, user.name);
