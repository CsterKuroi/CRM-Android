...
import com.example.spinel.myapplication.DateSlider.labeler.bpmLabeler;
...
public class ScrollLayout extends LinearLayout {
    ...
    private bpmLabeler mLabeler;
    private OnScrollListener listener;
    private bpmTimeView mCenterView;

    public ScrollLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.setWillNotDraw(false);
        rightShadow = getContext().getResources().getDrawable(R.drawable.bpm_right_shadow);
        leftShadow = getContext().getResources().getDrawable(R.drawable.bpm_left_shadow);
        ...
        try {
            ...
            mLabeler = (bpmLabeler)ctor.newInstance(labelerFormat);
        } catch (Exception e) {
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            bpmTimeView ttv = mLabeler.createView(getContext(), i == centerIndex);
            addView((View)ttv, lp);
        }
        ...
        mCenterView = (bpmTimeView)getChildAt(centerIndex);
        ...
        for (int i = centerIndex + 1; i < childCount; i++) {
            bpmTimeView lastView = (bpmTimeView)getChildAt(i - 1);
            bpmTimeView thisView = (bpmTimeView)getChildAt(i);
    ...
    private void setTime(long time, int loops) {
        ...
        bpmTimeView currelem = (bpmTimeView)getChildAt(pos);
    ...
    protected void moveElements(int steps) {
        ...
        for (int i = start; i != end; i += incr) {
            bpmTimeView tv = (bpmTimeView)getChildAt(i);
            int index = i - steps;
            if (index >= 0 && index < getChildCount()) {
                tv.setVals((bpmTimeView)getChildAt(index));
            } else {
